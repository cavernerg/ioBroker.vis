...
function calcProject(objects, projects, instance, result, callback) {
    ...
    objects.readFile(`vis.${instance}`, `/${project.file}/vis-views.json`, (err, data) => {
        ...
        if (dps && dps.IDs) {
            // The character class used to be [.\\s] - an escaped backslash plus the
            // LETTER s, not whitespace. Live result: vis.0.datapoints._potify instead of
            // Spotify, icon_ instead of icons, Jarvi__mobil instead of Jarvis.mobile.
