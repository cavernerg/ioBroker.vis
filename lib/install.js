...
function copyFileSync(source, target) {
    ...
    // This runs for the widgets/ tree of EVERY installed adapter on every single start
    // of vis - on this installation 85 files / 4.5 MB, almost always byte for byte the
    // same. Writing them anyway is pure flash wear. Compare first; the length check
    // settles the common case without a second buffer walk.
    try {
        if (fs.existsSync(targetFile)) {
            const old = fs.readFileSync(targetFile);
            if (old.length === data.length && old.equals(data)) {
                return;
            }
        }
    } catch (e) {
        // unreadable target - just overwrite it
    }
...
function syncWidgetSets(onlyLocal) {
            ...
            for (let d = 0; d < dirs.length; d++) {
                if (dirs[d].match(/^iobroker\./i) && fs.existsSync(`${__dirname}/../../../../${dirs[d]}/widgets/`)) {
                    ...
                    if (!sets.find(s => s.name === name)) {
                        ...
                        // pack was parsed above and then dropped: native.always and
                        // native.dependencies of a widget set found here were ignored
                        sets.push({path: `${__dirname}/../../../../${dirs[d]}`, name: dirs[d].toLowerCase(), pack});
                    }
                }
            }
        } catch (e) {
            // was swallowed without a word; a missing directory is the normal case
            if (e.code !== 'ENOENT') {
                console.warn(`Cannot read "${__dirname}/../../../../": ${e}`);
            }
        }
    }
    ...
    // main.js checks this very file with existsSync before reading it; here it was read
    // unguarded, so a missing config.js threw instead of simply being written.
    const configFile = `${__dirname}/../www/js/config.js`;
    const oldText = fs.existsSync(configFile) ? fs.readFileSync(configFile).toString() : '';
    if (oldText !== text) {
